...
class ValidOrderFrame(BaseFrame):
    ...
    def create_valid_order(self):
        ...
        Button(self, text="Payer en carte", command=self._root_frame.pay).grid(row=3, column=0, sticky='w')
        Button(self, text="Payer en espèce", command=self._root_frame.pay).grid(row=3, column=1, sticky='w')
        Button(self, text="Modifier commande", fg="green", command=self.back).grid(row=3, column=2, sticky='w')